...
def error_code_to_bytes(data):
    """
    Encode an error code into CayenneLPP,
    and return as a byte buffer
    """
    logging.debug("lpp_error_code_to_bytes")
    if not isinstance(data, tuple):
        data = (data,)
    if not len(data) == 1:
        raise AssertionError()
...
# def error_codes_to_bytes(data):
#     """
#     Encode error codes into CayenneLPP,
#     and return as a byte buffer
#     """
#     logging.debug("lpp_error_codes_to_bytes")
#     if not isinstance(data, tuple):
#         data = (data,)
#     if not len(data) > 0:
#         raise AssertionError()
#     l = len(data)
#     print("length of error array: " + l)
#     buf = bytearray(l + 1) # 1 extra for the byte indicating the length
#     buf[0] = l
#     for i in range(0, l):
#         buf[i + 1] = data[0][i]
#     return buf
...
LPP_TYPES = [
    ...
    LppType(201, 'Battery Voltage', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(202, 'PM25 Standard', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(203, 'PM100 Standard', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(204, 'PM25 Environmental', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(205, 'PM100 Environmental', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(206, 'Gas CO', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(207, 'Gas NO2', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(208, 'PM1 Concentration', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(209, 'PM25 Concentration', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(210, 'PM100 Concentration', 2, 1,
            lpp_analog_io_from_bytes, lpp_analog_io_to_bytes),
    LppType(255, 'Error Code', 1, 1,
            error_code_to_bytes, error_code_to_bytes),
    # LppType(255, 'Error Codes', 9, 8,
    #         error_codes_to_bytes, error_codes_to_bytes),
]
